...
const socialLinksConfig = {
  whatsapp: {
    baseUrl: "https://wa.me/",
    message:
      "Hi%20John,%20I%20saw%20your%20portfolio%20and%20would%20like%20to%20connect!",
    phone: "254723396228",
...
async function submitContactForm(formData) {
  try {
    ...
    return {
      success: true,
      message: "Message received! (Demo mode - form data logged to console)",
    };
  }
}
...
function validateSocialLink(link) {
  ...
  if (config) {
    ...
    if (href !== expectedUrl) {
      console.warn(
        `Social link URL mismatch for ${platform}. Expected: ${expectedUrl}, Got: ${href}`
      );
    }
  }
...
function handleNetworkChange(event) {
  ...
  if (event.type === "offline") {
    socialLinks.forEach((link) => {
      link.setAttribute(
        "aria-label",
        link.getAttribute("aria-label") + " (Offline)"
      );
      link.style.opacity = "0.6";
    });
  } else {
    socialLinks.forEach((link) => {
      const originalLabel = link
...
function trackSocialLinkClick(platform, url) {
  ...
  try {
    const existingData = JSON.parse(
      localStorage.getItem("socialLinkClicks") || "[]"
    );
